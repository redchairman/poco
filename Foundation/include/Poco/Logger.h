...
public:
	...
	void log(const Exception& exc, const char* file, int line, const char* func);
		/// Logs the given exception with priority PRIO_ERROR.	
		///
	...
	void fatal(const std::string& msg, const char* file, int line, const char* func);
		/// If the Logger's log level is at least PRIO_FATAL,
		/// creates a Message with priority PRIO_FATAL
		/// and the given message text and sends it
		/// to the attached channel.
	...
	void critical(const std::string& msg, const char* file, int line, const char* func);
		/// If the Logger's log level is at least PRIO_CRITICAL,
		/// creates a Message with priority PRIO_CRITICAL
		/// and the given message text and sends it
		/// to the attached channel.
	...
	void error(const std::string& msg, const char* file, int line, const char* func);
		/// If the Logger's log level is at least PRIO_ERROR,
		/// creates a Message with priority PRIO_ERROR
		/// and the given message text and sends it
		/// to the attached channel.
	...
	void warning(const std::string& msg, const char* file, int line, const char* func);
		/// If the Logger's log level is at least PRIO_WARNING,
		/// creates a Message with priority PRIO_WARNING
		/// and the given message text and sends it
		/// to the attached channel.
	...
	void notice(const std::string& msg, const char* file, int line, const char* func);
		/// If the Logger's log level is at least PRIO_NOTICE,
		/// creates a Message with priority PRIO_NOTICE
		/// and the given message text and sends it
		/// to the attached channel.
	...
	void information(const std::string& msg, const char* file, int line, const char* func);
		/// If the Logger's log level is at least PRIO_INFORMATION,
		/// creates a Message with priority PRIO_INFORMATION
		/// and the given message text and sends it
		/// to the attached channel.
	...
	void debug(const std::string& msg, const char* file, int line, const char* func);
		/// If the Logger's log level is at least PRIO_DEBUG,
		/// creates a Message with priority PRIO_DEBUG
		/// and the given message text and sends it
		/// to the attached channel.
...
protected:
	...
	void log(const std::string& text, Message::Priority prio, const char* file, int line, const char* func);
...
// convenience macros
//
#define poco_fatal(logger, fmt, ...) \
	if ((logger).fatal()) (logger).fatal(Poco::format((fmt), ##__VA_ARGS__), __FILE__, __LINE__, __FUNCTION__); else (void) 0

#define poco_fatal_f1(logger, fmt, arg1) \
	if ((logger).fatal()) (logger).fatal(Poco::format((fmt), arg1), __FILE__, __LINE__, __FUNCTION__); else (void) 0

#define poco_fatal_f2(logger, fmt, arg1, arg2) \
	if ((logger).fatal()) (logger).fatal(Poco::format((fmt), (arg1), (arg2)), __FILE__, __LINE__, __FUNCTION__); else (void) 0

#define poco_fatal_f3(logger, fmt, arg1, arg2, arg3) \
	if ((logger).fatal()) (logger).fatal(Poco::format((fmt), (arg1), (arg2), (arg3)), __FILE__, __LINE__, __FUNCTION__); else (void) 0

#define poco_fatal_f4(logger, fmt, arg1, arg2, arg3, arg4) \
	if ((logger).fatal()) (logger).fatal(Poco::format((fmt), (arg1), (arg2), (arg3), (arg4)), __FILE__, __LINE__, __FUNCTION__); else (void) 0

#define poco_critical(logger, fmt, ...) \
	if ((logger).critical()) (logger).critical(Poco::format((fmt), ##__VA_ARGS__), __FILE__, __LINE__, __FUNCTION__); else (void) 0

#define poco_critical_f1(logger, fmt, arg1) \
	if ((logger).critical()) (logger).critical(Poco::format((fmt), (arg1)), __FILE__, __LINE__, __FUNCTION__); else (void) 0

#define poco_critical_f2(logger, fmt, arg1, arg2) \
	if ((logger).critical()) (logger).critical(Poco::format((fmt), (arg1), (arg2)), __FILE__, __LINE__, __FUNCTION__); else (void) 0

#define poco_critical_f3(logger, fmt, arg1, arg2, arg3) \
	if ((logger).critical()) (logger).critical(Poco::format((fmt), (arg1), (arg2), (arg3)), __FILE__, __LINE__, __FUNCTION__); else (void) 0

#define poco_critical_f4(logger, fmt, arg1, arg2, arg3, arg4) \
	if ((logger).critical()) (logger).critical(Poco::format((fmt), (arg1), (arg2), (arg3), (arg4)), __FILE__, __LINE__, __FUNCTION__); else (void) 0

#define poco_error(logger, fmt, ...) \
	if ((logger).error()) (logger).error(Poco::format((fmt), ##__VA_ARGS__), __FILE__, __LINE__, __FUNCTION__); else (void) 0

#define poco_error_f1(logger, fmt, arg1) \
	if ((logger).error()) (logger).error(Poco::format((fmt), (arg1)), __FILE__, __LINE__, __FUNCTION__); else (void) 0

#define poco_error_f2(logger, fmt, arg1, arg2) \
	if ((logger).error()) (logger).error(Poco::format((fmt), (arg1), (arg2)), __FILE__, __LINE__, __FUNCTION__); else (void) 0

#define poco_error_f3(logger, fmt, arg1, arg2, arg3) \
	if ((logger).error()) (logger).error(Poco::format((fmt), (arg1), (arg2), (arg3)), __FILE__, __LINE__, __FUNCTION__); else (void) 0

#define poco_error_f4(logger, fmt, arg1, arg2, arg3, arg4) \
	if ((logger).error()) (logger).error(Poco::format((fmt), (arg1), (arg2), (arg3), (arg4)), __FILE__, __LINE__, __FUNCTION__); else (void) 0

#define poco_warning(logger, fmt, ...) \
	if ((logger).warning()) (logger).warning(Poco::format((fmt), ##__VA_ARGS__), __FILE__, __LINE__, __FUNCTION__); else (void) 0

#define poco_warning_f1(logger, fmt, arg1) \
	if ((logger).warning()) (logger).warning(Poco::format((fmt), (arg1)), __FILE__, __LINE__, __FUNCTION__); else (void) 0

#define poco_warning_f2(logger, fmt, arg1, arg2) \
	if ((logger).warning()) (logger).warning(Poco::format((fmt), (arg1), (arg2)), __FILE__, __LINE__, __FUNCTION__); else (void) 0

#define poco_warning_f3(logger, fmt, arg1, arg2, arg3) \
	if ((logger).warning()) (logger).warning(Poco::format((fmt), (arg1), (arg2), (arg3)), __FILE__, __LINE__, __FUNCTION__); else (void) 0

#define poco_warning_f4(logger, fmt, arg1, arg2, arg3, arg4) \
	if ((logger).warning()) (logger).warning(Poco::format((fmt), (arg1), (arg2), (arg3), (arg4)), __FILE__, __LINE__, __FUNCTION__); else (void) 0
	
#define poco_notice(logger, fmt, ...) \
	if ((logger).notice()) (logger).notice(Poco::format((fmt), ##__VA_ARGS__), __FILE__, __LINE__, __FUNCTION__); else (void) 0

#define poco_notice_f1(logger, fmt, arg1) \
	if ((logger).notice()) (logger).notice(Poco::format((fmt), (arg1)), __FILE__, __LINE__, __FUNCTION__); else (void) 0

#define poco_notice_f2(logger, fmt, arg1, arg2) \
	if ((logger).notice()) (logger).notice(Poco::format((fmt), (arg1), (arg2)), __FILE__, __LINE__, __FUNCTION__); else (void) 0

#define poco_notice_f3(logger, fmt, arg1, arg2, arg3) \
	if ((logger).notice()) (logger).notice(Poco::format((fmt), (arg1), (arg2), (arg3)), __FILE__, __LINE__, __FUNCTION__); else (void) 0

#define poco_notice_f4(logger, fmt, arg1, arg2, arg3, arg4) \
	if ((logger).notice()) (logger).notice(Poco::format((fmt), (arg1), (arg2), (arg3), (arg4)), __FILE__, __LINE__, __FUNCTION__); else (void) 0

#define poco_information(logger, fmt, ...) \
	if ((logger).information()) (logger).information(Poco::format((fmt), ##__VA_ARGS__), __FILE__, __LINE__, __FUNCTION__); else (void) 0

#define poco_information_f1(logger, fmt, arg1) \
	if ((logger).information()) (logger).information(Poco::format((fmt), (arg1)), __FILE__, __LINE__, __FUNCTION__); else (void) 0

#define poco_information_f2(logger, fmt, arg1, arg2) \
	if ((logger).information()) (logger).information(Poco::format((fmt), (arg1), (arg2)), __FILE__, __LINE__, __FUNCTION__); else (void) 0

#define poco_information_f3(logger, fmt, arg1, arg2, arg3) \
	if ((logger).information()) (logger).information(Poco::format((fmt), (arg1), (arg2), (arg3)), __FILE__, __LINE__, __FUNCTION__); else (void) 0

#define poco_information_f4(logger, fmt, arg1, arg2, arg3, arg4) \
	if ((logger).information()) (logger).information(Poco::format((fmt), (arg1), (arg2), (arg3), (arg4)), __FILE__, __LINE__, __FUNCTION__); else (void) 0

#if defined(_DEBUG) || defined(POCO_LOG_DEBUG)
	#define poco_debug(logger, fmt, ...) \
		if ((logger).debug()) (logger).debug(Poco::format((fmt), ##__VA_ARGS__), __FILE__, __LINE__, __FUNCTION__); else (void) 0

	#define poco_debug_f1(logger, fmt, arg1) \
		if ((logger).debug()) (logger).debug(Poco::format((fmt), (arg1)), __FILE__, __LINE__, __FUNCTION__); else (void) 0

	#define poco_debug_f2(logger, fmt, arg1, arg2) \
		if ((logger).debug()) (logger).debug(Poco::format((fmt), (arg1), (arg2)), __FILE__, __LINE__, __FUNCTION__); else (void) 0

	#define poco_debug_f3(logger, fmt, arg1, arg2, arg3) \
		if ((logger).debug()) (logger).debug(Poco::format((fmt), (arg1), (arg2), (arg3)), __FILE__, __LINE__, __FUNCTION__); else (void) 0

	#define poco_debug_f4(logger, fmt, arg1, arg2, arg3, arg4) \
		if ((logger).debug()) (logger).debug(Poco::format((fmt), (arg1), (arg2), (arg3), (arg4)), __FILE__, __LINE__, __FUNCTION__); else (void) 0

	#define poco_trace(logger, fmt, ...) \
		if ((logger).trace()) (logger).trace(Poco::format((fmt), ##__VA_ARGS__), __FILE__, __LINE__, __FUNCTION__); else (void) 0

	#define poco_trace_f1(logger, fmt, arg1) \
		if ((logger).trace()) (logger).trace(Poco::format((fmt), (arg1)), __FILE__, __LINE__, __FUNCTION__); else (void) 0

	#define poco_trace_f2(logger, fmt, arg1, arg2) \
		if ((logger).trace()) (logger).trace(Poco::format((fmt), (arg1), (arg2)), __FILE__, __LINE__, __FUNCTION__); else (void) 0

	#define poco_trace_f3(logger, fmt, arg1, arg2, arg3) \
		if ((logger).trace()) (logger).trace(Poco::format((fmt), (arg1), (arg2), (arg3)), __FILE__, __LINE__, __FUNCTION__); else (void) 0

	#define poco_trace_f4(logger, fmt, arg1, arg2, arg3, arg4) \
		if ((logger).trace()) (logger).trace(Poco::format((fmt), (arg1), (arg2), (arg3), (arg4)), __FILE__, __LINE__, __FUNCTION__); else (void) 0
#else
	#define poco_debug(logger, fmt, ...)
	#define poco_debug_f1(logger, fmt, arg1)
	#define poco_debug_f2(logger, fmt, arg1, arg2)
...
inline void Logger::log(const std::string& text, Message::Priority prio, const char* file, int line, const char* func)
{
	if (_level >= prio && _pChannel)
	{
		_pChannel->log(Message(_name, text, prio, file, line, func));
	}
}
...
inline void Logger::fatal(const std::string& msg, const char* file, int line, const char* func)
{
	log(msg, Message::PRIO_FATAL, file, line, func);
}
...
inline void Logger::critical(const std::string& msg, const char* file, int line, const char* func)
{
	log(msg, Message::PRIO_CRITICAL, file, line, func);
}
...
inline void Logger::error(const std::string& msg, const char* file, int line, const char* func)
{
	log(msg, Message::PRIO_ERROR, file, line, func);
}
...
inline void Logger::warning(const std::string& msg, const char* file, int line, const char* func)
{
	log(msg, Message::PRIO_WARNING, file, line, func);
}
...
inline void Logger::notice(const std::string& msg, const char* file, int line, const char* func)
{
	log(msg, Message::PRIO_NOTICE, file, line, func);
}
...
inline void Logger::information(const std::string& msg, const char* file, int line, const char* func)
{
	log(msg, Message::PRIO_INFORMATION, file, line, func);
}
...
inline void Logger::debug(const std::string& msg, const char* file, int line, const char* func)
{
	log(msg, Message::PRIO_DEBUG, file, line, func);
}
...
inline void Logger::trace(const std::string& msg, const char* file, int line, const char* func)
{
	log(msg, Message::PRIO_TRACE, file, line, func);
}
